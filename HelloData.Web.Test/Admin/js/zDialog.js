...
$.DiagShow = function (width, height, name, url) {
...
$(".submitLoading").html("数据正在处理...");
//function openwindow(width, height, name, url){ 
//         window.showModalDialog(url,window,"status:no;scroll:no;dialogWidth:"+width+"px;dialogHeight:"+height+"px");
//       }


$.DiagShow = function (width, height, name, url, funCallback) {
    var diag = new Dialog();
    if (width == null || width <= 100) {
        diag.Width = 420;
    }
    ...
    if (typeof funCallback == 'function') {
        diag.CancelEvent = function () {
            ...
            if (diag.innerFrame == null || diag.innerFrame.contentWindow.document.getElementById('isok') == null
            ) {
                diag.close();
                return false;
            }
            if (diag.innerFrame.contentWindow.document.getElementById('isok').value != "");
            {
                funCallback();
            }
            diag.close();
