        ...
        public override string CreatePageString(string tablename, string colums, string where, string order, int pageindex, int pagesize, out string selcountstr)
        {
            ...
            if (pageindex == 0)
            {
                ...
                if (string.IsNullOrEmpty(order))
                    return (string.Format("SELECT " + top + " {0} from {1}   {2}    ;",
                        colums, tablename, where));
                else
                    return (string.Format("SELECT " + top + " {0} from {1}   {2} ORDER BY {3}  ;",
            ...
            else
            {
                ...
                skip = pagesize * skip;
                if (string.IsNullOrEmpty(order))
                    return (string.Format("with temptbl as (SELECT ROW_NUMBER() OVER (ORDER BY {0})AS Row, {1} from {2}  O   {5} ) SELECT * FROM temptbl where Row between {3} and {4}",
